...
int task_compare_files(void* userData, const void* p1, const void* p2) {
    ...
    bool info1Base = strncmp(info1->name, "<현재 폴더>", LIST_ITEM_NAME_MAX) == 0 || strncmp(info1->name, "<현재 파일>", LIST_ITEM_NAME_MAX) == 0;
    bool info2Base = strncmp(info2->name, "<현재 폴더>", LIST_ITEM_NAME_MAX) == 0 || strncmp(info2->name, "<현재 파일>", LIST_ITEM_NAME_MAX) == 0;
...
static void task_populate_files_thread(void* arg) {
    ...
    if(R_SUCCEEDED(res = task_create_file_item(&baseItem, data->archive, data->path, 0, false))) {
        ...
        if(baseInfo->attributes & FS_ATTRIBUTE_DIRECTORY) {
            string_copy(baseItem->name, "<현재 폴더>", LIST_ITEM_NAME_MAX);
        } else {
            string_copy(baseItem->name, "<현재 파일>", LIST_ITEM_NAME_MAX);
        }
