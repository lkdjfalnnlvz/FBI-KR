    ...
    if(*(u16*) initialReadBlock == 0x2020) {
        ...
        if(R_SUCCEEDED(res = cia_get_title_id(&titleId, (u8*) initialReadBlock, installData->installInfo.bufferSize))) {
            ...
            if(R_SUCCEEDED(APT_CheckNew3DS(&n3ds)) && !n3ds && ((titleId >> 28) & 0xF) == 2) {
                ui_view* view = prompt_display_yes_no("확인", "이 타이틀은 New 3DS 전용입니다.\n계속할까요?", COLOR_TEXT, data, action_install_url_draw_top, action_install_url_n3ds_onresponse);
                if(view != NULL) {
...
static bool action_install_url_error(void* data, u32 index, Result res, ui_view** errorView) {
    ...
    if(strlen(url) > 38) {
        *errorView = error_display_res(data, action_install_url_draw_top, res, "URL을 통해 설치하는 데 실패함.\n%.35s...", url);
    } else {
        *errorView = error_display_res(data, action_install_url_draw_top, res, "URL을 통해 설치하는 데 실패함.\n%.38s", url);
    }
...
void action_install_url(const char* confirmMessage, const char* urls, const char* paths, void* userData,
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "URL 설치 데이터를 할당하는 데 실패했습니다.");
