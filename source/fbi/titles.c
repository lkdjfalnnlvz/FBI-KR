...
static list_item launch_title = {"타이틀 실행", COLOR_TEXT, action_launch_title};
static list_item delete_title = {"타이틀 삭제", COLOR_TEXT, action_delete_title};
static list_item delete_title_ticket = {"타이틀과 티켓 삭제", COLOR_TEXT, action_delete_title_ticket};
static list_item extract_smdh = {"SMDH 내보내기", COLOR_TEXT, action_extract_smdh};
static list_item import_seed = {"시드 가져오기", COLOR_TEXT, action_import_seed};
static list_item browse_save_data = {"세이브 데이터 둘러보기", COLOR_TEXT, action_browse_title_save_data};
static list_item import_save_data = {"세이브 데이터 가져오기", COLOR_TEXT, action_import_twl_save};
static list_item export_save_data = {"세이브 데이터 내보내기", COLOR_TEXT, action_export_twl_save};
static list_item erase_save_data = {"세이브 데이터 지우기", COLOR_TEXT, action_erase_twl_save};
static list_item import_secure_value = {"보안 값 가져오기", COLOR_TEXT, action_import_secure_value};
static list_item export_secure_value = {"보안 값 내보내기", COLOR_TEXT, action_export_secure_value};
static list_item delete_secure_value = {"보안 값 삭제하기", COLOR_TEXT, action_delete_secure_value};
...
static void titles_action_open(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "타이틀 작업 데이터를 할당하는 데 실패했습니다.");
    ...
    list_display("타이틀 작업", "A: 선택, B: 뒤로", data, titles_action_update, titles_action_draw_top);
}
...
static void titles_options_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(linked_list_size(items) == 0) {
        titles_options_add_entry(items, "게임 카트리지 보기", &listData->showGameCard);
        titles_options_add_entry(items, "SD 카드 보기", &listData->showSD);
        titles_options_add_entry(items, "낸드 보기", &listData->showNAND);
        titles_options_add_entry(items, "ID로 정렬", &listData->sortById);
        titles_options_add_entry(items, "이름으로 정렬", &listData->sortByName);
        titles_options_add_entry(items, "크기로 정렬", &listData->sortBySize);
    }
}

static void titles_options_open(titles_data* data) {
    list_display("설정", "A: 토글, B: 뒤로", data, titles_options_update, NULL);
}
...
static void titles_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(!listData->populated || (hidKeysDown() & KEY_X)) {
        ...
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "타이틀 목록 채우기를 시작하지 못했습니다.");
        }
    ...
    if(listData->populateData.finished && R_FAILED(listData->populateData.result)) {
        error_display_res(NULL, NULL, listData->populateData.result, "타이틀 목록 채우기에 실패했습니다다.");
...
void titles_open() {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "타이틀 데이터 할당에 실패했습니다.");
    ...
    list_display("타이틀", "A: 선택, B: 뒤로, X: 새로고침, Select: 설정", data, titles_update, titles_draw_top);
}
