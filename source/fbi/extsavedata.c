...
static void extsavedata_action_open(linked_list* items, list_item* selected) {
    ...
    list_display("추가 데이터 작업", "A: 선택, B: 취소(뒤로 가기)", data, extsavedata_action_update, extsavedata_action_draw_top);
}
...
static void extsavedata_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(!listData->populated || (hidKeysDown() & KEY_X)) {
        ...
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "추가 데이터 목록 채우기를 시작하지 못했습니다.");
        }
    ...
    if(listData->populateData.finished && R_FAILED(listData->populateData.result)) {
        error_display_res(NULL, NULL, listData->populateData.result, "추가 데이터 목록 채우기에 실패했습니다..");
...
void extsavedata_open() {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "추가 데이터를 할당하지 못했습니다.");
    ...
    list_display("추가 데이터", "A: 선택, B: 뒤로 가기, X: 새로고침, Select: 설정", data, extsavedata_update, extsavedata_draw_top);
}
